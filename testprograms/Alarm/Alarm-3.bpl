...
{
	alarmHour := alarmAlarmHour;
	alarmMin := alarmAlarmMin;
...
ensures (old(min + alarmLength) >= 60 && old(hour) < 23) ==> hour == old(hour) + 1 && min == old(min + alarmLength) - 60;
ensures (old(min + alarmLength) >= 60 && old(hour) == 23) ==> hour == 0 && min == old(min + alarmLength) - 60;
ensures hour < 24 && hour >=0 && min >= 0 && min < 60;
ensures alarmSignal == 0;
{
	alarmSignal := 1;
	if (min + alarmLength >= 60 && hour < 23){
